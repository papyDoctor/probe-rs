use super::{
    function_die::FunctionDie, get_sequential_key, halt_locations::HaltLocations,
    unit_info::UnitInfo, unit_info::UnitIter, variable::*, DebugError, DebugRegisters,
    SourceLocation, StackFrame, VariableCache,
};
                        ...
                        while let Some((header, row)) = rows.next_row()? {
                            ...
                            if let Some(cur_line) = row.line() {
                                if cur_line.get() == line {
                                    ...
                                    return HaltLocations::new(self, row.address(), None);
                                }
                            }
                        }
                    }
